...
async function getFleetChild(req: Request, res: Response) {
    try {
        const result = await fleetModel.getFleetChild(req.params.id);
        res.status(200).json({ status: 1, message: "ok", response: result })
    } catch (err) {
        res.status(500).json({ status: 0, message: "failed from server", response: err })
    }
...
async function deleteFleet(req: Request, res: Response) {
    try {
        ...
        await fleetModel.deleteFleet(req.params.id, body);
