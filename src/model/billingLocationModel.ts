...
async function getBillingLocationTable(index: number, filter: string) {
    try {
        ...
        let result = await pool.request()
            ...
            .query(`
                EXEC chonTest..getBillingLocationTable @name = @name, @firstIndex = @firstIndex, @lastIndex = @lastIndex

                SELECT COUNT(*) AS count_data
                FROM chonTest..BillingLocation
    ...
    try {
        ...
        let result = await pool.request()
            ...
            .query(`
                ...
                FROM chonTest..BillingLocation
                WHERE billing_location_id = @billing_location_id AND isArchived = 0
                ...
                LEFT JOIN chonTest..BillingLocation BL
...
async function deleteBillingLocation(billingLocationId: string) {
    try {
        ...
        let result = await pool.request()
            ...
            .query(`
                UPDATE chonTest..BillingLocation
...
async function createBillingLocationData(body: any) {
    ...
    try {
        ...
        let billingLocationResult = await transaction.request()
            ...
            .query(`
                INSERT INTO chonTest..BillingLocation (name, tin, branch, customer_id, address_id, person_id, create_by, create_date, isArchived)
                OUTPUT INSERTED.billing_location_id
                VALUES (@name, @tin, @branch, @customer_id, @address_id, @person_id, @create_by, @create_date, @isArchived)
            `)
