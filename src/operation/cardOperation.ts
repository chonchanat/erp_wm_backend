...
export async function getCardTable(transaction: any, index: number, filter: string) {
    return await transaction.request()
        ...
        .query(`
            ...
            INSERT @cardTable
            EXEC WDMT_MasterData..sp_filterCard @value = @value, @person_id = NULL, @firstIndex = @firstIndex, @lastIndex = @lastIndex
            EXEC WDMT_MasterData..sp_formatCardTable @cardTable = @cardTable, @firstIndex = @firstIndex
...
export async function createCardNew(transaction: any, card: Card, person_id: string | number | null, action_by: string | number, datetime: object) {
    return await transaction.request()
        ...
        .query(`
        EXEC WDMT_MasterData..sp_insert_card @card_code_id = @card_code_id, @value = @value, @person_id = @person_id,
            @action_by = @action_by, @action_date = @action_date
    `)
}

export async function updateCard(transaction: any, card_id: string | number, card: Card, action_by: string | number, datetime: object) {
    return await transaction.request()
        .input('card_id', sql.INT, card_id)
        .input('card_code_id', sql.INT, card.card_code_id)
        .input('value', sql.NVARCHAR, card.value !== "" ? card.value : null)
        ...
        .input('action_date', sql.DATETIME, datetime)
        .query(`
            EXEC WDMT_MasterData..sp_update_card @card_id = @card_id, @card_code_id = @card_code_id, 
...
export async function deleteCard(transaction: any, card_id: string | number, action_by: string | number, datetime: object) {
    return await transaction.request()
        ...
        .input('action_date', sql.DATETIME, datetime)
        .query(`
            EXEC WDMT_MasterData..sp_delete_card @card_id = @card_id, @action_by = @action_by, @action_date = @action_date    
        `)
}