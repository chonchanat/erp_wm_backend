...
export async function getFleetTable(transaction: any, index: number, filter: string) {
    return await transaction.request()
        ...
        .query(`
            ...
            EXEC WDMT_MasterData..sp_filterFleet @fleet_name = @fleet_name, @customer_id = NULL, @vehicle_id = NULL, @firstIndex = @firstIndex, @lastIndex = @lastIndex
            EXEC WDMT_MasterData..sp_formatFleetTable @fleetTable = @fleetTable, @firstIndex = @firstIndex

            SELECT COUNT(*) AS count_data
            FROM WDMT_MasterData..Fleet
...
export async function getFleetData(transaction: any, fleet_id: string) {
    return await transaction.request()
        ...
        .query(`
            ...
            FROM WDMT_MasterData..Fleet F
            LEFT JOIN WDMT_MasterData..Fleet PF
            ON F.parent_fleet_id = PF.fleet_id
            WHERE F.fleet_id = @fleet_id

            DECLARE @customerTable IdType
            INSERT INTO @customerTable
            EXEC WDMT_MasterData..sp_filterCustomer @customer_name = '%',@fleet_id = @fleet_id, @person_id = NULL, @vehicle_id = NULL, @firstIndex = 0, @lastIndex = 0
            EXEC WDMT_MasterData..sp_formatCustomerTable @customerTable = @customerTable, @firstIndex = 1

            DECLARE @personTable IdType
            INSERT INTO @personTable
            EXEC WDMT_MasterData..sp_filterPerson @fullname = '%', @customer_id = NULL, @fleet_id = @fleet_id, @vehicle_id = NULL, @user_id = NULL, @firstIndex = 0, @lastIndex = 0
            EXEC WDMT_MasterData..sp_formatPersonTable @personTable = @personTable, @firstIndex = 1

            DECLARE @vehicleTable IdType
            INSERT INTO @vehicleTable 
            EXEC WDMT_MasterData..sp_filterVehicle @license_plate = '%', @customer_id = NULL, @fleet_id = @fleet_id, @package_id = NULL, @firstIndex = 0, @lastIndex = 0
...
export async function deleteFleet(transaction: any, fleet_id: string, action_by: number, datetime: object) {
    return await transaction.request()
        ...
        .query(`
            EXEC WDMT_MasterData..sp_delete_fleet @fleet_id = @fleet_id, 
...
export async function createFleetNew(transaction: any, fleet: Fleet, action_by: string | number, datetime: object) {
    return await transaction.request()
        ...
        .input('action_date', sql.DATETIME, datetime)
        .query(`
        EXEC WDMT_MasterData..sp_insert_fleet @fleet_name = @fleet_name, @parent_fleet_id = @parent_fleet_id,
            @action_by = @action_by, @action_date = @action_date
    `)
}
...
export async function updateFleet(transaction: any, fleet_id: string | number, fleet: Fleet, action: string | number, datetime: object) {
    return await transaction.request()
        ...
        .input('action_date', sql.DATETIME, datetime)
        .query(`
        EXEC WDMT_MasterData..sp_update_fleet @fleet_id = @fleet_id, @fleet_name = @fleet_name,
