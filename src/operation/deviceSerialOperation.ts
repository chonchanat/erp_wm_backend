...
export async function createDeviceSerialNew(transaction: any, deviceSerial: DeviceSerial, action_by: string | number, datetime: object) {
    return await transaction.request()
        .input('serial_id', sql.NVARCHAR, deviceSerial.serial_id !== "" ? deviceSerial.serial_id : null)
        .input('imei_serial', sql.NVARCHAR, deviceSerial.imei_serial !== "" ? deviceSerial.imei_serial : null)
