...
export async function getAddressTable(transaction: any, index: number, filter: string) {
    return await transaction.request()
        ...
        .query(`
            ...
            EXEC WDMT_MasterData..sp_filterAddress @location = @location, @customer_id = NULL, @person_id = NULL, @firstIndex = @firstIndex, @lastIndex = @lastIndex
            EXEC WDMT_MasterData..sp_formatAddressTable @addressTable = @addressTable, @firstIndex = @firstIndex
...
export async function getAddressData(transaction: any, address_id: string) {
    return await transaction.request()
        ...
        .query(`
            ...
            FROM WDMT_MasterData..Address A
            WHERE A.address_id = @address_id

            SELECT
                am.address_type_code_id,
                m.value as address_type
            FROM WDMT_MasterData..Address_MasterCode am
...
export async function createAddressNew(transaction: any, address: Address, action_by: string | number, datetime: object) {
    return await transaction.request()
        ...
        .query(`
            EXEC WDMT_MasterData..sp_insert_address @name = @name, @house_no = @house_no, @village_no = @village_no,
...
export async function updateAddress(transaction: any, address_id: string | number, address: Address, action_by: string | number, datetime: object) {
    return await transaction.request()
        ...
        .input('action_date', sql.DATETIME, datetime)
        .query(`
            EXEC WDMT_MasterData..sp_update_address @address_id = @address_id, @name = @name, @house_no = @house_no,
...
export async function deleteAddressData(transaction: any, address_id: string, action_by: number, datetime: object) {
    return await transaction.request()
        ...
        .input('action_date', sql.DATETIME, datetime)
        .query(`
            EXEC WDMT_MasterData..sp_delete_address @address_id = @address_id, @action_by = @action_by, @action_date = @action_date
        `)
}
...
export async function getAddressProvince(transaction: any) {
    return await transaction.request()
        .query(`
            ...
            FROM WDMT_MasterData..AddressModel
