...
export async function getVehicleModelTable(transaction: any, index: number, filter: string) {
    return await transaction.request()
        ...
        .query(`
            ...
            EXEC WDMT_MasterData..sp_filterVehicleModel @filter = @filter, @firstIndex = @firstIndex, @lastIndex = @lastIndex
            EXEC WDMT_MasterData..sp_formatVehicleModelTable @vehicleModelTable = @vehicleModelTable, @firstIndex = @firstIndex

            SELECT COUNT(*) AS count_data
            FROM WDMT_MasterData..VehicleModel
...
export async function createVehicleModelData(transaction: any, vehicleModel: VehicleModel, action_by: string | number, datetime: object) {
    return await transaction.request()
        ...
        .query(`
            EXEC WDMT_MasterData..sp_insert_vehicleModel @brand = @brand, @model = @model, 
...
export async function updateVehicleModelData(transaction: any, vehicle_model_id: string, vehicleModel: VehicleModel, action_by: string | number, datetime: object) {
    return await transaction.request()
        ...
        .input('action_date', sql.DATETIME, datetime)
        .query(`
            EXEC WDMT_MasterData..sp_update_vehicleModel @vehicle_model_id = @vehicle_model_id, @brand = @brand, @model = @model,
                @action_by = @action_by, @action_date = @action_date
        `)
}
...
export async function deleteVehicleModelData(transaction: any, vehicle_model_id: string, action_by: string | number, datetime: object) {
    return await transaction.request()
        ...
        .input('action_date', sql.DATETIME, datetime)
        .query(`
            EXEC WDMT_MasterData..sp_delete_vehicleModel @vehicle_model_id = @vehicle_model_id, 
                @action_by = @action_by, @action_date = @action_date
        `)
}
